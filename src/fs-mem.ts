...
export class OpenDirectory extends Fd {
  ...
  path_open(
    ...
    const { ret: lookupRet, entry: lookupEntry } =
      this.dir.get_entry_for_path(path);
    ...
    if (entry == null) {
      ...
      if ((oflags & wasi.OFLAGS_CREAT) == wasi.OFLAGS_CREAT) {
        const { ret: createRet, entry: newEntry } =
          this.dir.create_entry_for_path(
            path_str,
...
export class Directory extends Inode {
  ...
  get_entry_for_path(path: Path): { ret: number; entry: Inode | null } {
    const result = path.parts.reduce<{ ret: number; entry: Inode | null }>(
      (acc, component) => {
        if (acc.entry === null) {
          return acc;
        }
...
export class DevDirectory extends Fd {
  private devices: Map<string, Fd>;
  private prestat_name: string;
